...
- Começou por ser uma experiência no meio académico no final da decada de 60
...
- **Startup:** Academica & Investigação (DARPA)
...
- Questões que se colocamn:
	- Quem é o dono da Internet?
	- Quem é o responsável pela sua gestão?
- Solução: 
	- Criação de um orgão de gestão: `The Internet Socienty`
...
## Glossário:
- **Routers:** dispositivos que transportam pacoes entre conexões físicas a redes diferente
...
- É uma rede **best effort:** a rede faz o melhor que pode, com os recursos que têm. Se o melhor que puder fazer não chega, a qualidade do serviço é diminuida
...
- especifica que ações são executadas quandos as mensagens são recebidas
...
Os protocolos funcionam nos extermos da rede e não no seu intermédio/caminhos da rede.
...
- Cada `tier` representa uma àrea de prestação de serviços
...
- A relação/importânica hierárquica que possuem
- As relações que cada uma das entidades estabelece com outras entidades
- Número de entidades envolvidas

# Methods for inforamtion forwading in networks

## Circuit Swtiching
- São reservados recursos entre os _end-pointss_ para a conexão durante todo o tempo em que a mesma dure
- Podem ser necessários processos auxiliares para inicar e finalizar a conexão
	- Estes processos são **externos** ao sistema de conexão, tendo de existir antes de uma chamada ser efetuada e depois de a chamda terminar
- Todo o processo de conexão é feito offline

Antigamente todos os métodos de routing eram por _circuit swiching_: correspondiam a alguém conectar fisicamente dois cabos, que efetuada uma ligação entre o destinatário da chamada e quem a iniciou. Mais tarde passou a ser um processo eletromecânico.
...
- Quando ocorre uma congestão da rede e não são possíveis reservar mais circuitos, os recursos existentes mantêm as suas propriedades 
	...
	- Uma comunicação é bloqueada se a rede não possui capacidde de resposta
...
- A informaçaão é dividida em `packets`(pacotes) 
...
- A diferenção entre `circuit/cell switching` e `packet switching` é que é os pacotes têm um tamanho fixo no `packet switching`
...
- Cada mensagem pode ser armazenada em cada nó pelo tempo necssário
	- p.e, email
...
## Connection- oriented
- É necessário estabelecer uam conexão entre o emissor e o recetor antes de enviar a informação para a rede
...
- 	Nem sempre implica que seja necessário **reservar recursos** para a conexãoa
...
- Os cilentes comunicam com uma **entidade hierarquicamente** acima deles
...
### Cookies
	- a informação reside no lado do clientee
	...
	- Problema: E se eu quiser mudar uma página para Portuguêss, supondo que tenho um servidor com capacidde multi-língua, como mantenho todas as outras páginas que abrir depois dessa em Português?
	...
	> This is how we design a state awareness system in a statelees web engine


### Cache
	-	É informação que já foi pedida ao servidor e foi armazenada temporariamente 
	-	O cliente informa o servidor que já possui informação, indicando a timestamp.
...
- Os `peers` são:
	- identificáveis
	- contactáveis
	- capzes de estabelecer as comunicações necessárias, como efetuar pedidos e responder a pedidos
- Modelo útil para a partilha de conteudo
...
Comparação com o modelo cliente-sevidor: 
...
- `broadcast`
	- um para todos
	- a camada física das redes é sempre boradcast (ou quase sempre)
...
- **Com/Sem requisitos temporais fixos**
	...
	- Requerem que seja definido o atraso temporal de uma comunicação
		- facil numa rede ponto a ponto
		- muito díficil numa rede onde a implementação física e a lógica não são diretamente relacionadas

- **Symetrical/Assymetrical**

A maior parte das comunicações hoje em dia são assimétricas

- Em conexões bidirecionais, os recursos disponíveis em differentes direções podem ser diferentes
...
- A `client station` efetua um pedido a um serviço e recebe do servidor.
	...
	- Exemplo de serviços: browser/webserver e client/server email
...
Pode facilemnte variar entre 1 e 5. A largura de banda que é vendida a cada utilizador não é a largura de banda real que esse utilizador tem direito em todas as condições. O que acontece é uma partilha de recursos entre os vários clientes, uma vez que a probabilidade de os utilizadores todos precisarem da largura de banda máxima ao mesmo tempo é muito baixa, o dimensionamento do sistema para este caso é demasiado caro e conduz ao desperdício de recursos
